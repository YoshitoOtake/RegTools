...
M = size(xstart, 2);
counteval = zeros(M,1); countiter = zeros(M,1);
irun = 0;
...
% 	   num2str(N)]);
%   else
  if ~(all(size(insigma) == [1 1]) || all(size(insigma) == [N 1]))
...
%   end
  sigma = max(insigma,[],1);
  if(length(sigma)==1)
      insigma = repmat( insigma, 1, M);
      sigma = repmat( sigma, 1, M );              % overall standard deviation
      stopTolUpX = repmat( stopTolUpX, 1, M );              % overall standard deviation
  end
  pc = zeros(N,M); ps = zeros(N,M);  % evolution paths for C and sigma

  if length(insigma) == 1
    insigma = insigma * ones(N,M) ;
  end
...
%   startseed = randn('state');         % for retrieving in saved variables
  ...
  diagD = zeros(N,M); diagC = zeros(N,M);
  B = zeros(N,N,M); BD = zeros(N,N,M); C = zeros(N,N,M);
  arz = zeros(N,lambda,M);
  arxvalid = zeros(N,lambda,M);
  arx = zeros(N,lambda,M);
  lbounds_mat = permute( repmat(lbounds, [1 1 lambda]), [1 3 2] );
  ubounds_mat = permute( repmat(ubounds, [1 1 lambda]), [1 3 2] );

  for m=1:M
      diagD(:,m) = insigma(:,m)/max(insigma(:,m));      % diagonal matrix D defines the scaling
      diagC(:,m) = diagD(:,m).^2; 
      if flgDiagonalOnly ~= 1            % use at some point full covariance matrix
        B(:,:,m) = eye(N,N);                      % B defines the coordinate system
        BD(:,:,m) = B(:,:,m).*repmat(diagD(:,m)',N,1);        % B*D for speed up only
        C(:,:,m) = diag(diagC(:,m));                   % covariance matrix == BD*(BD)'
      end
% % % %       if flgDiagonalOnly
% % % %         B = 1; 
% % % %       end

      fitness(m).hist=NaN*ones(1,10+ceil(3*10*N/lambda)); % history of fitness values
      fitness(m).histsel=NaN*ones(1,10+ceil(3*10*N/lambda)); % history of fitness values
      fitness(m).histbest=[]; % history of fitness values
      fitness(m).histmedian=[]; % history of fitness values
      
      % Initialize boundary handling
      bnd(m).isactive = any(lbounds(:,m) > -Inf) || any(ubounds(:,m) < Inf); 
      if bnd(m).isactive
        if any(lbounds(:,m)>ubounds(:,m))
          error('lower bound found to be greater than upper bound');
        end
        [xmean(:,m) ti] = xintobounds(xmean(:,m), lbounds(:,m), ubounds(:,m)); % just in case
        if any(ti)
          warning('Initial point was out of bounds, corrected');
        end
        bnd(m).weights = zeros(N,1);         % weights for bound penalty
        %%%% We do NOT use scaling
        % scaling is better in axis-parallel case, worse in rotated
        bnd(m).flgscale = 0; % scaling will be omitted if zero 
    % % % %     if bnd(m).flgscale ~= 0 
    % % % %       bnd(m).scale = diagC/mean(diagC);
    % % % %     else
          bnd(m).scale = ones(N,1);
    % % %     end

        idx = (lbounds(:,m) > -Inf) | (ubounds(:,m) < Inf);
        if length(idx) == 1
          idx = idx * ones(N,1);
        end
        bnd(m).isbounded = zeros(N,1);
        bnd(m).isbounded(find(idx)) = 1; 
        maxdx = min(maxdx, (ubounds(:,m) - lbounds(:,m))/2);
    % % % %     if any(sigma(:,1)*sqrt(diagC(:,1)) > maxdx)
    ...
    %     end
        bnd(m).dfithist = 1;              % delta fit for setting weights
        bnd(m).aridxpoints = [];          % remember complete outside points
        bnd(m).arfitness = [];            % and their fitness
        bnd(m).validfitval = 0;
        bnd(m).iniphase = 1;
      end
  end
  ...
  if irun == 1 
    ...
    out.solutions.bestever.f = Inf(1,M);  % for simpler comparison below
...
% % % %     end
    ...
    for m=1:M
        fitness(m).hist(1)=NaN; 
        fitness(m).histsel(1)=NaN; 
    end
  end
...
isActive = ones(M, 1);  % flags indicating if the restart is still continuing (1) or not (0) 
while any(isActive)
  % set internal parameters
  %%%% parameters common for all restarts
  if countiter(1) == 0 || lambda ~= lambda_last
    if countiter(1) > 0 && floor(log10(lambda)) ~= floor(log10(lambda_last)) ...
          && flgdisplay
      disp(['  lambda = ' num2str(lambda)]);
      lambda_hist(:,end+1) = [countiter(1)+1; lambda];
    else
      lambda_hist = [countiter(1)+1; lambda]; 
    end
    lambda_last = lambda;
    ...
    elseif N > 98 && flgdisplay && countiter(1) == 0
...
%   fprintf('Generation %d, %d restarts are active\n', size(lambda_hist,2), sum(isActive));
...
% % % %   flush;
  ...
  active_restarts = find(isActive);
%   disp(['active_restarts: ' num2str(active_restarts') ', countiter: ' num2str(countiter')]);
 
% % % %   fitness.raw = repmat(NaN, 1, lambda + noiseReevals);

    %%%% We always use parallel evaluation
  % parallel evaluation
% % % %   if flgEvalParallel
      invalid = false(M, lambda);
      for m=1:length(active_restarts)
          cur = active_restarts(m);
          countiter(cur) = countiter(cur) + 1; 

          %%%% always do NOT use diagonal only option for low-dimensional
          %%%% problem (N < ~100)
% % % %           if ~flgDiagonalOnly
%             arx(:,:,cur) = repmat(xmean(:,cur), 1, lambda) + sigma(cur) * (BD(:,:,cur) * arz(:,:,cur)); % Eq. (1)
            arz(:,:,cur) = randn(N,lambda);     % use new random sequence
            arx(:,:,cur) = xmean(:,cur)*ones(1, lambda) + sigma(cur) * (BD(:,:,cur) * arz(:,:,cur)); % Eq. (1)
% % % %           else
...
% % % % 
    ...
    % % % %       end
          ...
          if ~bnd(cur).isactive
            arxvalid(:,:,cur) = arx(:,:,cur);
          else
            arxvalid(:,:,cur) = xintobounds_multi(arx(:,:,cur), lbounds_mat(:,:,cur), ubounds_mat(:,:,cur));
          end
          ...
          if(size(varargin,2)==2 && ~isempty(varargin{2}.cmaes_inclusionTest))
              ...
              isInclude = feval(varargin{2}.cmaes_inclusionTest, arxvalid(:,:,cur), cur, varargin{:});
%               fprintf('include %d/%d\n', length(find(isInclude)), length(isInclude));
              
              total_samples = length(isInclude); total_include = sum(isInclude);
              invalid(cur,:) = ~isInclude;
              invalid_sample_thresh = 1e-4;
              while(any(invalid(cur,:)) && (total_include/total_samples)>invalid_sample_thresh)
%                   timer_id = tic;
                  arz(:,invalid(cur,:),cur) = randn(N,sum(invalid(cur,:))); % resample
                  arx(:,invalid(cur,:),cur) = xmean(:,cur)*ones(1, sum(invalid(cur,:))) + sigma(cur) * (BD(:,:,cur) * arz(:,invalid(cur,:),cur)); % Eq. (1)
                  if ~bnd(cur).isactive
                    arxvalid(:,invalid(cur,:),cur) = arx(:,invalid(cur,:),cur);
                  else
                    arxvalid(:,invalid(cur,:),cur) = xintobounds_multi(arx(:,invalid(cur,:),cur), lbounds_mat(:,invalid(cur,:),cur), ubounds_mat(:,invalid(cur,:),cur));
                  end
                  include = feval(varargin{2}.cmaes_inclusionTest, arxvalid(:,invalid(cur,:),cur), cur, varargin{:});
                  total_samples = total_samples + length(include); total_include = total_include + sum(include);
                  retry_index = find(invalid(cur,:));
                  invalid(cur, retry_index(include)) = false;
%                   fprintf('%.3f%% (%d/%d) included, pop size: %d, %.3f sec\n', (total_include/total_samples)*100, total_include, total_samples, lambda, toc(timer_id));
              end
              if((total_include/total_samples)<=invalid_sample_thresh),
                  invalid(cur,:) = true;
                  arxvalid(:,invalid(cur,:),cur) = Inf;
                  fprintf('start %d failed, %d included out of %d tries\n', cur, total_include, total_samples); 
              end
          end
      ...
      fitness_raw_all = feval(fitfun, arxvalid(:,:,active_restarts), active_restarts, varargin{:});
      fitness_raw_all(invalid(active_restarts,:)) = Inf;
      for m=1:length(active_restarts)
          cur = active_restarts(m);
          fitness(cur).raw = fitness_raw_all(m,:);
          counteval(cur) = counteval(cur) + sum(~isnan(fitness(cur).raw)); 
      end
% % % %       fitness.raw = feval(fitfun, arxvalid, varargin{:}); 
% % % %       countevalNaN = countevalNaN + sum(isnan(fitness.raw));
% % % %   end
  
  %%%% We won't use NaN, so deleted below
...
% % % %   end
  ...
  for m=1:length(active_restarts)
      ...
      fitness(cur).sel = fitness(cur).raw; 

      % ----- handle boundaries -----
      if 1 < 3 && bnd(cur).isactive
        % Get delta fitness values
        val = myprctile(fitness(cur).raw, [25 75]);
        % more precise would be exp(mean(log(diagC)))
        val = (val(2) - val(1)) / N / mean(diagC(:,cur)) / sigma(cur)^2;
        %val = (myprctile(fitness(cur).raw, 75) - myprctile(fitness(cur).raw, 25)) ...
        %    / N / mean(diagC) / sigma^2;
        % Catch non-sensible values 
        if ~isfinite(val)
          warning('Non-finite fitness range');
          val = max(bnd(cur).dfithist);  
        elseif val == 0 % happens if all points are out of bounds
          val = min(bnd(cur).dfithist(bnd(cur).dfithist>0));  % seems not to make sense, given all solutions are out of bounds
        elseif bnd(cur).validfitval == 0 % flag that first sensible val was found
          bnd(cur).dfithist = [];
          bnd(cur).validfitval = 1;
        end

        % Store delta fitness values
        if length(bnd(cur).dfithist) < 20+(3*N)/lambda
          bnd(cur).dfithist = [bnd(cur).dfithist val];
        else
          bnd(cur).dfithist = [bnd(cur).dfithist(2:end) val];
        end

        [tx ti]  = xintobounds(xmean(:,cur), lbounds(:,cur), ubounds(:,cur));

        % Set initial weights
        if bnd(cur).iniphase 
          if any(ti) 
            bnd(cur).weights(find(bnd(cur).isbounded)) = 2.0002 * median(bnd(cur).dfithist);
    % % % % 	if bnd(cur).flgscale == 0 % scale only initial weights then
          dd = diagC(:,cur); 
          idx = find(bnd(cur).isbounded); 
          dd = dd(idx) / mean(dd); %  remove mean scaling
          bnd(cur).weights(idx) = bnd(cur).weights(idx) ./ dd; 
    % % % % 	end
    ...
    % % % %     end
        ...
        bnd(cur).arpenalty = (bnd(cur).weights ./ bnd(cur).scale)' * (arxvalid(:,:,cur) - arx(:,:,cur)).^2; 

        fitness(cur).sel = fitness(cur).raw + bnd(cur).arpenalty;
    ...
    % % % % %   end
      ...
      % Sort by fitness 
      [fitness(cur).raw, fitness(cur).idx] = sort(fitness(cur).raw); 
      [fitness(cur).sel, fitness(cur).idxsel] = sort(fitness(cur).sel);  % minimization
      fitness(cur).hist(2:end) = fitness(cur).hist(1:end-1);    % record short history of
      fitness(cur).hist(1) = fitness(cur).raw(1);               % best fitness values
      if length(fitness(cur).histbest) < 120+ceil(30*N/lambda) || ...
           (mod(countiter(cur), 5) == 0  && length(fitness(cur).histbest) < 2e4)  % 20 percent of 1e5 gen.
        fitness(cur).histbest = [fitness(cur).raw(1) fitness(cur).histbest];          % best fitness values
        fitness(cur).histmedian = [median(fitness(cur).raw) fitness(cur).histmedian]; % median fitness values
      else
        fitness(cur).histbest(2:end) = fitness(cur).histbest(1:end-1); 
        fitness(cur).histmedian(2:end) = fitness(cur).histmedian(1:end-1); 
        fitness(cur).histbest(1) = fitness(cur).raw(1);           % best fitness values
        fitness(cur).histmedian(1) = median(fitness(cur).raw);    % median fitness values
      end
      fitness(cur).histsel(2:end) = fitness(cur).histsel(1:end-1); % record short history of
      fitness(cur).histsel(1) = fitness(cur).sel(1);               % best sel fitness values

      % Calculate new xmean, this is selection and recombination 
      xold(:,cur) = xmean(:,cur); % for speed up of Eq. (2) and (3)
      cmean = 1;  % 1/min(max((lambda-1*N)/2, 1), N);  % == 1/kappa
      xmean(:,cur) = (1-cmean) * xold(:,cur) + cmean * arx(:,fitness(cur).idxsel(1:mu),cur)*weights; 
      zmean = arz(:,fitness(cur).idxsel(1:mu),cur)*weights;%==D^-1*B'*(xmean-xold)/sigma
      if mu == 1
        fmean = fitness(cur).sel(1);
      else
      ...
      ps(:,cur) = (1-cs)*ps(:,cur) + sqrt(cs*(2-cs)*mueff) * (B(:,:,cur)*zmean);          % Eq. (4)
      hsig = norm(ps(:,cur))/sqrt(1-(1-cs)^(2*countiter(cur)))/chiN < 1.4 + 2/(N+1);
      if flg_future_setting
        hsig = sum(ps(:,cur).^2) / (1-(1-cs)^(2*countiter(cur))) / N < 2 + 4/(N+1); % just simplified
      end
    ...
    %  hsig = 1;
      ...
      pc(:,cur) = (1-cc)*pc(:,cur) ...
            + hsig*(sqrt(cc*(2-cc)*mueff)/sigma(cur)/cmean) * (xmean(:,cur)-xold(:,cur));     % Eq. (2)
      if hsig == 0
...
% % % %                 * (diagC .* (arz(:,fitness(cur).idxsel(1:mu)).^2 * weights));
% % % %     %             * (repmat(diagC,1,mu) .* arz(:,fitness(cur).idxsel(1:mu)).^2 * weights);
% % % %           diagD(:,cur) = sqrt(diagC); % replaces eig(C)
% % % %         else
%           arpos = (arx(:,fitness(cur).idxsel(1:mu),cur)-repmat(xold(:,cur),1,mu)) / sigma(cur);
          arpos = (arx(:,fitness(cur).idxsel(1:mu),cur)-xold(:,cur)*ones(1,mu)) / sigma(cur);
          ...
          if flgActiveCMA > 0
            ...
            arzneg = arz(:,fitness(cur).idxsel(lambda:-1:lambda - neg.mu + 1),cur);
            ...
            if mod(flgActiveCMA, 10) == 1 && 1 - neg.ccov * arnorms(idxnorms).^2 * weights < neg.minresidualvariance
              ...
              if neg.ccovfinal < neg.ccov
                disp(['active CMA at iteration ' num2str(countiter(cur)) ...
                     ': max EV ==', num2str([maxeigenval, neg.ccov, neg.ccovfinal])]);
              end
            end
            % xmean = xold;  % the distribution does not degenerate!? 
            % update C
            C(:,:,cur) = (1-ccov1-ccovmu+neg.alphaold*neg.ccovfinal+(1-hsig)*ccov1*cc*(2-cc)) * C(:,:,cur) ... % regard old matrix 
                + ccov1 * pc(:,cur)*pc(:,cur)' ...     % plus rank one update
            ...
            C(:,:,cur) = (1-ccov1-ccovmu+(1-hsig)*ccov1*cc*(2-cc)) * C(:,:,cur) ... % regard old matrix 
                + ccov1 * pc(:,cur)*pc(:,cur)' ...     % plus rank one update
                + ccovmu ...             % plus rank mu update
                  * arpos * (weights*ones(1,N) .* arpos');
...
% % % %         end
      ...
      if 11 < 2 && ~flgscience  
        ...
        if sum(ps(:,cur).^2)/N > 1.5 + 10*(2/N)^.5 && ...
            fitness(cur).histsel(1) > max(fitness(cur).histsel(2:3))
          ps(:,cur) = ps(:,cur) * sqrt(N*(1+max(0,log(sum(ps(:,cur).^2)/N))) / sum(ps(:,cur).^2));
          if flgdisplay
            disp(['Momentum in ps removed at [niter neval]=' ...
                  num2str([countiter(cur) counteval(cur)]) ']']);
          end
        end
      end
    ...
    % % % %   else
        ...
        sigma(cur) = sigma(cur) * exp(min(1, (sqrt(sum(ps(:,cur).^2))/chiN - 1) * cs/damps));             % Eq. (5)
    % % % %   end
    ...
    % % % %   end
      ...
      if ~flgDiagonalOnly && (ccov1+ccovmu+neg.ccov) > 0 && mod(countiter(cur), 1/(ccov1+ccovmu+neg.ccov)/N/10) < 1
        C(:,:,cur)=triu(C(:,:,cur))+triu(C(:,:,cur),1)'; % enforce symmetry to prevent complex numbers
        [B(:,:,cur),tmp] = eig(C(:,:,cur));     % eigen decomposition, B==normalized eigenvectors
                              % effort: approx. 15*N matrix-vector multiplications
        diagD(:,cur) = diag(tmp); 

        if any(~isfinite(diagD(:,cur)))
        ...
        if stopOnWarnings
          ...
          warning(['Iteration ' num2str(countiter(cur)) ...
               ': Eigenvalue (smaller) zero']);
          diagD(diagD(:,cur)<0,cur) = 0;
          tmp = max(diagD(:,cur))/1e14;
          C(:,:,cur) = C(:,:,cur) + tmp*eye(N,N); diagD(:,cur) = diagD(:,cur) + tmp*ones(N,1); 
        end
        end
        if max(diagD(:,cur)) > 1e14*min(diagD(:,cur)) 
        if stopOnWarnings
          stopflag(end+1) = {'warnconditioncov'};
        else
          warning(['Iteration ' num2str(countiter(cur)) ': condition of C ' ...
               'at upper limit' ]);
          tmp = max(diagD(:,cur))/1e14 - min(diagD(:,cur));
          C(:,:,cur) = C(:,:,cur) + tmp*eye(N,N); diagD(:,cur) = diagD(:,cur) + tmp*ones(N,1); 
        end
        end

        diagC(:,cur) = diag(C(:,:,cur)); 
        diagD(:,cur) = sqrt(diagD(:,cur)); % D contains standard deviations now
        % diagD = diagD / prod(diagD)^(1/N);  C = C / prod(diagD)^(2/N);
        BD(:,:,cur) = B(:,:,cur).*(ones(N,1)*(diagD(:,cur)')); % O(n^2)
      end % if mod
      ...
      if 1 < 2 && sigma(cur) > 1e10*max(diagD(:,cur)) && sigma(cur) > 8e14 * max(insigma(:,cur))
        fac = sigma(cur); % / max(diagD);
        sigma(cur) = sigma(cur)/fac;
        pc(:,cur) = fac * pc(:,cur);
        diagD(:,cur) = fac * diagD(:,cur); 
        if ~flgDiagonalOnly
          C(:,:,cur) = fac^2 * C(:,:,cur); % disp(fac);
          BD(:,:,cur) = B(:,:,cur) .* repmat(diagD(:,cur)',N,1); % O(n^2), but repmat might be inefficient todo?
        end
        diagC(:,cur) = fac^2 * diagC(:,cur); 
      end
    ...
    % % % %   end
      ...
      if any(sigma(cur)*sqrt(diagC(:,cur)) > maxdx)
        sigma(cur) = min(maxdx ./ sqrt(diagC(:,cur)));
      ...
      if any(sigma(cur)*sqrt(diagC(:,cur)) < mindx)
        sigma(cur) = max(mindx ./ sqrt(diagC(:,cur))) * exp(0.05+cs/damps); 
        %warning(['Iteration ' num2str(countiter) ': coordinate axis std ' ...
        %         'deviation at lower limit of ' num2str(mindx)]);
        % stopflag(end+1) = {'mincoorddev'};;
      end
      % Adjust too low coordinate axis deviations
      if any(xmean(:,cur) == xmean(:,cur) + 0.2*sigma(cur)*sqrt(diagC(:,cur))) 
        if stopOnWarnings
          stopflag(end+1) = {'warnnoeffectcoord'};
        else
          warning(['Iteration ' num2str(countiter(cur)) ': coordinate axis std ' ...
                    'deviation too low' ]);
% % % %           if flgDiagonalOnly
% % % %             diagC = diagC + (ccov1_sep+ccovmu_sep) * (diagC .* ...
% % % %                                                       (xmean == xmean + 0.2*sigma(cur)*sqrt(diagC)));
% % % %           else
            C(:,:,cur) = C(:,:,cur) + (ccov1+ccovmu) * diag(diagC(:,cur) .* ...
                                          (xmean(:,cur) == xmean(:,cur) + 0.2*sigma(cur)*sqrt(diagC(:,cur))));
% % % %           end
          sigma(cur) = sigma(cur) * exp(0.05+cs/damps); 
        end
      end
    ...
    % % % %     tmp = 0.1*sigma*diagD; 
    % % % %   else
        tmp = 0.1*sigma(cur)*BD(:,1+floor(mod(countiter(cur),N)),cur);
    % % % %   end
      if all(xmean(:,cur) == xmean(:,cur) + tmp)
        i = 1+floor(mod(countiter(cur),N));
        if stopOnWarnings
        stopflag(end+1) = {'warnnoeffectaxis'};
        else
          warning(['Iteration ' num2str(countiter(cur)) ...
               ': main axis standard deviation ' ...
               num2str(sigma(cur)*diagD(i,cur)) ' has no effect' ]);
          sigma(cur) = sigma(cur) * exp(0.2+cs/damps); 
        end
      end
      % Adjust step size in case of equal function values (flat fitness)
      % isequalfuncvalues = 0; 
      if fitness(cur).sel(1) == fitness(cur).sel(1+ceil(0.1+lambda/4))
        % isequalfuncvalues = 1; 
        if stopOnEqualFunctionValues
          arrEqualFunvals = [countiter(cur) arrEqualFunvals(1:end-1)];
          % stop if this happens in more than 33%
          if arrEqualFunvals(end) > countiter(cur) - 3 * length(arrEqualFunvals)
            stopflag(end+1) = {'equalfunvals'}; 
          end
        else
          if flgWarnOnEqualFunctionValues
            warning(['Iteration ' num2str(countiter(cur)) ...
             ': equal function values f=' num2str(fitness(cur).sel(1)) ...
             ' at maximal main axis sigma ' ...
            num2str(sigma(cur)*max(diagD(:,cur)))]);
          end
          sigma(cur) = sigma(cur) * exp(0.2+cs/damps); 
        end
      end
      % Adjust step size in case of equal function values
      if countiter(cur) > 2 && myrange([fitness(cur).hist fitness(cur).sel(1)]) == 0  
        if stopOnWarnings
        stopflag(end+1) = {'warnequalfunvalhist'};
        else
          warning(['Iteration ' num2str(countiter(cur)) ...
               ': equal function values in history at maximal main ' ...
               'axis sigma ' num2str(sigma(cur)*max(diagD(:,cur)))]);
          sigma(cur) = sigma(cur) * exp(0.2+cs/damps); 
        end
      end
      ...
      out.solutions.recentbest.x = arxvalid(:, fitness(cur).idx(1),cur);
      out.solutions.recentbest.f = fitness(cur).raw(1);
      out.solutions.recentbest.evals = counteval(cur) + fitness(cur).idx(1) - lambda;
      out.solutions.recentworst.x = arxvalid(:, fitness(cur).idx(end),cur);
      out.solutions.recentworst.f = fitness(cur).raw(end);
      out.solutions.recentworst.evals = counteval(cur) + fitness(cur).idx(end) - lambda;
      if fitness(cur).hist(1) < out.solutions.bestever.f
        out.solutions.bestever.x = arxvalid(:, fitness(cur).idx(1),cur);
        out.solutions.bestever.f = fitness(cur).hist(1);
        out.solutions.bestever.evals = counteval(cur) + fitness(cur).idx(1) - lambda;
        bestever = out.solutions.bestever;
      end

      % Set stop flag
      if isinf(fitness(cur).raw(1)), stopflag(end+1) = {'fitness all infinite'}; end
      if fitness(cur).raw(1) <= stopFitness, stopflag(end+1) = {'fitness'}; end
      if counteval(cur) >= stopMaxFunEvals, stopflag(end+1) = {'maxfunevals'}; end
      if countiter(cur) >= stopMaxIter, stopflag(end+1) = {'maxiter'}; end
%       if all(sigma(cur)*(max(abs(pc(:,cur)), sqrt(diagC(:,cur)))) < stopTolX) 
%         stopflag(end+1) = {'tolx'};
      if all(sigma(cur)*diagD(:,cur) < stopTolX) 
        stopflag(end+1) = {'tolx_diagD'};
      end
      if any(sigma(cur)*sqrt(diagC(:,cur)) > stopTolUpX(cur)) 
        stopflag(end+1) = {'tolupx'};
      end
      if sigma(cur)*max(diagD(:,cur)) == 0  % should never happen
        stopflag(end+1) = {'bug'};
      end
      if countiter(cur) > 2 && myrange([fitness(cur).sel fitness(cur).hist]) <= stopTolFun 
        stopflag(end+1) = {'tolfun'};
      end
      if countiter(cur) >= length(fitness(cur).hist) && myrange(fitness(cur).hist) <= stopTolHistFun 
        stopflag(end+1) = {'tolhistfun'};
      end
      l = floor(length(fitness(cur).histbest)/3);
      if 1 < 2 && stopOnStagnation && ...  % leads sometimes early stop on ftablet, fcigtab
          countiter(cur) > N * (5+100/lambda) && ...  
          length(fitness(cur).histbest) > 100 && ... 
          median(fitness(cur).histmedian(1:l)) >= median(fitness(cur).histmedian(end-l:end)) && ...
          median(fitness(cur).histbest(1:l)) >= median(fitness(cur).histbest(end-l:end))
        stopflag(end+1) = {'stagnation'};
      end

      if counteval(cur) >= stopFunEvals || countiter(cur) >= stopIter
    ...
    % % % %   end
      ...
      if(~isempty(stopflag)), isActive(cur) = 0; end
    % % % %   % ----- output generation -----
...
%       fprintf('start%d, xmean: (%f, %f, %f, %f, %f, %f)\n', cur, xmean(:,cur));
%       fprintf('start%d, countiter = %d, bnd.weights = %f, %f, %f, %f, %f, %f, sigma = %f\n', cur, countiter(cur), bnd(cur).weights, sigma(cur));
%       fprintf('start%d, countiter = %d, cc = %f, ccov1 = %f, ccovmu = %f, chiN = %f, sigma = %f\n', cur, countiter, cc, ccov1, ccovmu, chiN, sigma(cur));
  end
...
fmin = zeros(M,1); xmin = zeros(N,M);
for m=1:M
    fmin(m) = fitness(m).raw(1);
    xmin(:,m) = arxvalid(:, fitness(m).idx(1), m); % Return best point of last generation in each restart.
end
...
function [x, idx] = xintobounds_multi(x, lbounds_mat, ubounds_mat)
%
% x should be a matrix consisting of column vectors
%
  if ~isempty(lbounds_mat)
      idx = x < lbounds_mat;
      x(idx) = lbounds_mat(idx);
  else
    idx = 0;
  end
  if ~isempty(ubounds_mat)
      idx2 = x > ubounds_mat;
      x(idx2) = ubounds_mat(idx2);
  else
    idx2 = 0;
  end
  idx = idx2-idx; 
