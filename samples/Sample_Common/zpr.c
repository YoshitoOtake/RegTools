...
zprMotion(int x, int y)
{
    ...
    if (_modifier == GLUT_ACTIVE_SHIFT && _mouseLeft) {
      intensity_window_current += ((float)dx)*0.01*intensity_addjustment_scaling;
      intensity_level_current  += ((float)dy)*0.01*intensity_addjustment_scaling;
      if(intensity_window_current<0)  intensity_window_current = 0;
      UpdateWindowLevel();
      changed = true;
    } else if (_modifier == GLUT_ACTIVE_CTRL && _mouseLeft) {
      ray_casting_thresh += ((float)dy)*1*intensity_addjustment_scaling;
      UpdateWindowLevel();
      changed = true;
    } else if (_modifier == GLUT_ACTIVE_SHIFT && _mouseRight) {
      changed = true;
...
//        glTranslatef(-zprReferencePoint[0],-zprReferencePoint[1],-zprReferencePoint[2]);
 ...
 //       glTranslatef(-zprReferencePoint[0],-zprReferencePoint[1],-zprReferencePoint[2]);
      ...
      glLoadIdentity();
      glTranslatef((px-_dragPosX)*mouseRightScale, (py-_dragPosY)*mouseRightScale,(pz-_dragPosZ)*mouseRightScale);
      glMultMatrixd(_matrix);
